...
유지보수
- SQL을 직접 다루면 엔티티에 필드를 하나만 추가해도 엄청난 양의 코드를 변경해야했다.
- 이런걸 JPA가 대신 해주므로 유지보수에 좋다.
- JPA가 패러다임 문제도 해결해주므로 유연하고 유지보수하기 좋은 도메인 모델을 편리하게 설계할 수 있다.

패러다임의 불일치 해결
- 패러다임 불일치는 심각한데, JPA가 해결해준다.

성능
- JPA는 애플리케이션과 데이터베이스 사이에서 다양한 성능 최적화 기회를 제공한다(그 둘 사이 계층에 있기 때문)
- 참고로 하이버네이트는 SQL 힌트 넣을수 있는 기능도 제공한다.

데이터 접근 추상화와 벤더 독립성
- 디비 종류, 벤더마다 기능이 달라서 디비를 바꾸면 코드를 바꾸는 경우도 생긴다.
- JPA는 특정 디비에 종속되지 않도록 해서 코드 변경이 최소화 되어있다.

표준
- JPA는 자바진영 ORM 표준 인터페이스라 다른 구현 기술로 손쉽게 변경 가능하다.

## 1.4 정리
SQL매퍼의 문제점을 심각하게 다뤄봤고 JPA는 그냥 다된다고 표현했다. 이제 어떻게 JPA는 그걸 가능하게 하는지 알아보자.
